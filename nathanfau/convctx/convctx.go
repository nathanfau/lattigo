...
type CtxSwitcher struct {
	...
	// KeyBytes is the serialized size of the three switching keys built below (the two ring-swap
	// keys, of degree 2N, and the CI relinearization key). It is measured here because the keys
	// themselves are private; a P sweep needs it.
	KeyBytes int
...
// MaskBytes is how many masks are cached and what they weigh. There is one per level (and per scale
// hop) a conversion has run at, built on demand, so the figure grows during a run.
func (c *CtxSwitcher) MaskBytes() (masks, bytes int) {
	for _, cache := range []map[maskKey]*rlwe.Plaintext{c.ptMask1, c.ptMask2} {
...
func (c *CtxSwitcher) StandardToCI(ctStd *rlwe.Ciphertext) (*rlwe.Ciphertext, error) {
	...
	*ctCI.MetaData = *ctBig.MetaData
	...
	// The fold doubled the encoded value, so the scale doubles with it.
