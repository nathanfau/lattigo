...
func TestTransciphering(t *testing.T) {
	...
	// This gives every round the same level profile (the previous MaxLevel start made round 1
	// behave differently from round 2). Round keys stay at MaxLevel so AddRoundKeyCanon can drop
	// them to the (deep) state level.
	const inputLevel = 5
